from bhrc_blockchain.utils.utils import (
    get_readable_time,
    get_random_quote,
    generate_address,
    render_template
...
def test_render_template_returns_template_response():

    app = FastAPI()

    @app.get("/test-template")
    def test_view(request: Request):
        return render_template("panel/login.html", request, {"user": "test_user"})
